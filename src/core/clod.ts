import { CLOD_KEY_PREFIX, CLOD_TYPE, ERROR_PREFIX, __DEV__ } from "../constants";
import type {
	AnyClod,
	AnyClodState,
	AsyncFunction,
...
export class Clod<T extends AnyClodState> {
	...
	make(value: StateValueTypeForSet<T>): T extends Maker<AsyncFunction> ? Promise<void> : void;
	...
	depClod(this: AnyClod, clod: AnyClod) {
		if (clod.key === this.key) {
			return;
		}
		// check circular getter
		if (Clod.isClodCircularDeps(this, clod)) {
			throw new Error(ERROR_PREFIX + "There is a circular reference pick. Please check it.");
		}
	...
	static isClodCircularDeps(source: AnyClod, target: AnyClod) {
		const deps = Clod.getClodDependencies(target);
		return deps.has(source);
	}

	static getClodDependencies(clod: AnyClod): Set<AnyClod> {
		const result = new Set<AnyClod>(clod.depClodSet);

		const addDepsToResult = (target: AnyClod) => {
			target.depClodSet.forEach((dep: AnyClod) => {
				if (result.has(dep)) return;
				result.add(dep);
				addDepsToResult(dep);
			});
		};

		addDepsToResult(clod);
